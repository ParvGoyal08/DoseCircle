...
| 1:40–2:05 | The Family dashboard tab: adherence, dose calendar, timing, who responds, refill | "The family sees how she is really doing: doses taken against the clinical 80% line, which slot is slipping, who responds fastest, and when the tablets run out." |
| 2:05–2:20 | Prescription photo → review screen, amber rows, tick each one, save | "A prescription photo is read by Amazon Textract and Claude on Bedrock. The codes are decoded by fixed rules, a guardrail strips anything like advice, and nothing is saved until a person checks every line." |
| 2:20–2:45 | **AWS console:** the Step Functions graph mid-execution, EventBridge Scheduler with Asia/Kolkata, the CloudWatch dashboard | "One Step Functions execution per dose. It waits for a tap for free, so this costs about two and a half rupees per parent per month." |
| 2:45–2:55 | Back to the three phones, then the repo | "Three languages, each reviewed by a native speaker. Reminders and family alerts only: never medical advice." |
