    ...
    <ol className="relative">
      {items.map((item, index) => {
        ...
        return (
          ...
          >
            {!last && <span aria-hidden className="absolute left-[15px] top-8 bottom-0 w-0.5 bg-line" />}
            <span className={cx("relative z-10 grid size-8 shrink-0 place-items-center rounded-full", look.dot, live && last && "ring-4 ring-haldi/40")}>
            ...
            <div className="min-w-0 flex-1 pt-0.5">
              ...
              <div className="mt-1.5 flex flex-wrap gap-1.5">
                {item.stateName && (
                  <code className="rounded-md border border-line bg-paper px-1.5 py-0.5 font-mono text-[11.5px] text-muted" title={t("timeline.stateTag")}>
                    {item.stateName}
                  </code>
                )}
                {item.authorizedBy?.map((policy) => (
                  <code key={policy} className="inline-flex items-center gap-1 rounded-md border border-taken/25 bg-taken-tint px-1.5 py-0.5 font-mono text-[11.5px] text-taken" title={t("timeline.allowedBy")}>
